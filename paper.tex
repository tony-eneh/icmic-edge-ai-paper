...
\title{Optimal Task Routing in Constrained Edge AI: Local Inference, Offload, or Safe Fallback under Uncertain Connectivity}
...
\begin{abstract}
Mobile and maritime networks increasingly rely on edge AI for situational awareness, anomaly detection, and local operational decisions. Yet these environments are characterized by intermittent connectivity, constrained compute, and heterogeneous link quality, which makes naive model execution brittle. This paper proposes an uncertainty-aware task routing policy for edge AI workloads that decides whether a task should execute locally, be forwarded to a better-connected node, or fall back to a safe rule-based mode when confidence is low or the network is degraded. The design treats uncertainty as an operational signal rather than only a model-quality statistic: uncertain predictions trigger routing and safety decisions before they affect downstream actions. The contribution is a lightweight systems design for resilient edge inference under realistic connectivity uncertainty, together with a compact evaluation methodology targeting latency, reliability, routing overhead, and fallback behavior. The paper is positioned for mobile and maritime deployments where operational continuity matters more than peak model accuracy.
\end{abstract}
...
The usual edge-computing framing emphasizes latency reduction, bandwidth savings, and energy efficiency. Those objectives remain important, but they are incomplete for mission-facing mobile systems. A vessel, unmanned surface vehicle, port-side gateway, or field inspection device may be forced to act while satellite backhaul is congested, while a relay is temporarily unavailable, or while a local model encounters inputs that differ from its training distribution. In such cases, the system needs a routing decision that is coupled to prediction confidence and communication state. A policy that always prefers the fastest path may amplify incorrect predictions, while a policy that always offloads may fail exactly when the network is most degraded.

We therefore treat task routing as a control problem with three possible outcomes. The node can use its local model, forward the task to another node with better resources or connectivity, or invoke a safe fallback routine. The fallback option is not intended to be more accurate than the AI model; rather, it is designed to be conservative, explainable, and available under degraded conditions. Examples include threshold rules for equipment alerts, rate-limited human notification, or holding a prior operational state until stronger evidence arrives.
...
Let each incoming task be represented as $x_i=(d_i,u_i,s_i)$, where $d_i$ is the sensor payload or feature vector, $u_i$ is the urgency class, and $s_i$ is optional state from the local platform. The local model produces a prediction $\hat{y}_i$ and a confidence estimate $c_i\in[0,1]$. The node also observes a queue load $q_i$, a link-quality estimate $\ell_i\in[0,1]$, and a peer availability indicator $a_i$. The action space is
\begin{equation}
    r_i \in \{\text{local}, \text{forward}, \text{fallback}\}.
\end{equation}
The routing objective is not a single scalar optimum in all deployments. In a safety-critical alerting task, avoiding low-confidence local decisions may dominate latency. In routine monitoring, latency and energy may be weighted more heavily. This motivates a policy that exposes simple thresholds rather than hiding the decision behind an opaque optimizer.

The main assumptions are modest. Each node can compute or receive a calibrated confidence proxy, maintain approximate queue and link-quality state from local telemetry, and access an application-defined fallback routine. The proposed routing layer does not invent the safe action; it decides when the safe action should be preferred.
...
Sensor tasks $\rightarrow$ local model and uncertainty estimator $\rightarrow$ routing policy $\rightarrow$ local completion / peer forwarding / safe fallback\vspace{1em}
}}
\caption{High-level routing architecture for resilient edge AI.}
\label{fig:arch}
\end{figure}

The router is intentionally placed after model scoring but before external actuation. This placement lets the node use model uncertainty without changing the model itself. It also allows application-specific fallback logic to remain separate from the learned model. For example, an onboard anomaly detector can still run as usual, but alerts below a confidence threshold may be converted into a low-priority inspection request instead of triggering an immediate automated response.

The forwarding path is best viewed as opportunistic rather than mandatory. A node may forward a task to a better-connected relay, a nearby gateway, or a peer with a stronger accelerator. If no acknowledgment is received within a bounded time, the task returns to the router and is either executed locally or sent to fallback depending on urgency and confidence. This prevents indefinite waiting on broken links.
...
The routing score combines three normalized signals: uncertainty, queue pressure, and link quality. We define uncertainty as $u_i=1-c_i$, local load as $\bar{q}_i$, and link quality as $\ell_i$. A simple deployable score for local execution is
\begin{equation}
    S_{local}=\alpha c_i - \beta \bar{q}_i + \gamma \kappa_i,
\end{equation}
where $\kappa_i$ captures urgency-dependent preference for local action. A forwarding score is
\begin{equation}
...
\begin{equation}
    S_{fallback}=\rho u_i + \eta (1-\ell_i) + \delta \phi_i,
\end{equation}
...
The experimental workload should include both routine and safety-sensitive tasks. Routine tasks test whether the router maintains throughput, while safety-sensitive tasks test whether uncertainty is converted into fallback decisions. A simple trace generator can model task arrivals with a Poisson process, attach urgency labels, and sample confidence values from distributions representing in-distribution and shifted inputs. Link conditions can be emulated by changing packet loss, delay, and peer availability across time windows.
...
The primary metrics are task latency, forwarding rate, fallback rate, and the fraction of low-confidence tasks that are prevented from executing locally. A secondary metric is routing overhead, because the controller must remain lightweight enough for field deployment. For safety-sensitive tasks, we also track unsafe local execution rate, defined as the fraction of tasks below the confidence floor that are still completed through the local AI path.
...
\begin{tabular}{p{0.22\columnwidth}p{0.33\columnwidth}p{0.31\columnwidth}}
\toprule
...
A practical evaluation can be built from a small emulation layer: synthetic task arrivals, configurable link delay, and four routing policies (always local, always forward, load-only, and uncertainty-aware). Each scenario can run for a fixed number of tasks, with identical random seeds across policies. The most informative plots are latency distributions, action-selection rates over time, and unsafe local execution rate under input shift. The expected result is that the uncertainty-aware policy retains competitive latency under good links while reducing harmful low-confidence local executions when conditions worsen.
